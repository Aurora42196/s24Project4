...
void createRevision(istream& fold, istream& fnew, ostream& frevision)
{
    ...
    if(newString.size() < 500) // 500 bytes
    {
        sequenceLength = 4;
    }
    else if(newString.size() > 30000) // 30KB
    {
        sequenceLength = 16; // Use a bigger sequence length for larger file sizes
    }
    else if(newString.size() > 50000) // 50KB
    {
        sequenceLength = 32;
    }
    ...
    size_t j = 0;
    size_t length = 0;
    for(; j <= newString.size();)
    {
        string newSeq = newString.substr(j, sequenceLength);
        
        if(hashTable.contains(newSeq, oldOffset))
        {
            // This loop checks beyond the length of the initial sequence to see if
        ...
        {
            length = 1;
            while (j + length < newString.size() /*&& newString[j+length] != newString[oldOffset]*/)
            {
...
//            while (j + length < newString.size() && oldOffset + length < oldString.size() && newString[j+length] != oldString[oldOffset + length])
//            {
//                length++;
//            }
